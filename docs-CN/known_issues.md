...
4. **[已修复] 速度平滑器闭环控制失败**
   ...
   - 根因: Ki=100 + 积分限幅被注释掉 → iout 无限累积；speed_rpm 用 uint16_t 解析导致负转速错误；串口接收缺少 \0 终止符导致解析越界
   - 修复: 分支 `stm32f4_fix` — 恢复积分限幅、修正 speed_rpm 符号、补全串口 \0、统一轮距参数、使用实际 DeltaTime 和 4 电机均值计算里程计
   - 状态: 代码已完成，待烧录验证（2026-04-20）
   - 来源: 2025-11 系列实车测试
...
18. **[已修复] 下位机 PID 急停倒转**
   - 描述: 急停逻辑会引起车轮明显反转。
   - 根因: B 键急停未清零 PID 积分器 + led_pink_blink() 阻塞 400ms + 缺少近零速释放阶段
   - 修复: 分支 `stm32f4_fix` — 重写为两阶段受控制动（PID 主动制动 + 近零速释放），清零积分器，非阻塞 LED
   - 状态: 代码已完成，待烧录验证（2026-04-20）
