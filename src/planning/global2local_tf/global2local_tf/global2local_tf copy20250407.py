...
class CoordinateTransformer(Node):
    ...
    def imu_callback(self, msg):
        """
        处理 /imu/data_raw 数据，用于计算初始朝向。
        """
        if self.initialized:
            return  # 如果已初始化，直接返回

        if self.start_time is None:
            self.start_time = time.time()  # 记录初始化开始时间
        ...
        try:
            ...
            if time.time() - self.start_time >= 3:
                avg_angle = sum(self.imu_angle_queue) / len(self.imu_angle_queue)
                self.orientation_angle = avg_angle
                # self.get_logger().info(f"Initial IMU orientation: {self.orientation_angle:.2f} degrees")
                
                # 如果 GNSS 原点也已经完成初始化，则标记为完成
