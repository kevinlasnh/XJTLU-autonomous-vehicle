...
- 从 `~/XJTLU-autonomous-vehicle/runtime-data/gnss/startid.txt` 读取起始校准点编号，支持 `1-4`。
...
- 计算出的偏移量写入 `~/XJTLU-autonomous-vehicle/runtime-data/gnss/gnss_offset.txt`。
- 校准后沿用原始消息头、状态、高度和协方差，只修正经纬度，然后发布到 `/gnss`。

## 运行时文件

- `~/XJTLU-autonomous-vehicle/runtime-data/gnss/startid.txt`
  - 启动前必须存在，内容为 `1-4` 之一。
- `~/XJTLU-autonomous-vehicle/runtime-data/gnss/gnss_offset.txt`
  - 校准成功后生成，两行分别是纬度偏移和经度偏移。
- 日志输出
  - 若由 `scripts/launch_with_logs.sh` 启动，日志写入当前 session 目录。
  - 否则回退到 `~/XJTLU-autonomous-vehicle/runtime-data/logs/gnss_calibration/`。
