...
- At runtime, only compiled artifacts under `~/fyp_runtime_data/gnss/current_scene/` are read
...
`~/fyp_runtime_data/` is decoupled from the code repository and currently contains:
...
- `~/fyp_runtime_data/gnss/current_route.yaml` (generated by `collect_gps_route.py`)
